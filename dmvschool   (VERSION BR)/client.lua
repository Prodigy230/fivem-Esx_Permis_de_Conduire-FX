...
function startttest()
        if TestDone == 0 or testblock == 1 then
			DrawMissionText2("~r~Bloqueado", 5000)		
		else
...
function EndDTest()
        if Error >= maxErrors then
			drawNotification("Você falhou\nvocê acumulou ".. Error.." ~r~Pontos de erros")
			EndTestTasks()
		else
...
function DTut()
	...
	DTutOpen = true
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "<b style='color:#1E90FF'>Examinador do Detran:</b> <br /><br /> Atualmente, estamos preparando seu veículo para o teste, enquanto isso você deve ler algumas linhas importantes.<br /><br /><b style='color:#87CEFA'>Limite de velocidade:</b><br />- Preste atenção ao tráfego e fique abaixo do <b style='color:#A52A2A'>limite</b> de velocidade<br /><br />- Até agora, você deve saber o básico, no entanto, tentaremos lembrá-lo sempre que você <b style='color:#DAA520'>entrar/sair</b> de uma área com um limite de velocidade permitido.",
            type = "alert",
            timeout = (15000),
            layout = "center",
            queue = "global"
		...
		Citizen.Wait(16500)
		TriggerEvent("pNotify:SendNotification",{
            text = "<b style='color:#1E90FF'>Examinador do Detran:</b> <br /><br /> Use o <b style='color:#DAA520'>recurso controle de cruzeiro</b> para evitar <b style='color:#87CEFA'>o excesso de velocidade</b>, ative isso durante o teste, pressionando a tecla <b style='color:#20B2AA'>X</b> no seu teclado.<br /><br /><b style='color:#87CEFA'>Avaliação:</b><br />- tente não bater o veículo ou superar o limite de velocidade permitido. Você receberá <b style='color:#A52A2A'>pontos por cada erro</b> sempre que você não seguir estas regras<br /><br />- Muitos <b style='color:#A52A2A'>pontos</b> acumulados resultarão em <b style='color:#A52A2A'>reprovação</b> do teste",
            type = "alert",
            timeout = (15000),
            layout = "center",
            queue = "global"
...
Citizen.CreateThread(function()
    while true do
		...
		TriggerEvent("pNotify:SendNotification",{
            text = "O veículo foi <b style='color:#B22222'>danificado!</b><br /><br />Cuidado!",
            type = "alert",
            timeout = (2000),
            layout = "bottomCenter",
            queue = "global"
	...
	if onTestEvent == 3 then
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),234.90780639648,-1345.3854980469, 29.542045593262, true) > 4.0001 then
		   DrawMarker(1,234.90780639648,-1345.3854980469, 29.542045593262,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Faça uma ~r~parada~s~ rápida para o cruzamento de ~y~pedestres", 5000)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),217.82102966309,-1410.5201416016,28.292112350464, true) > 4.0001 then
		   DrawMarker(1,217.82102966309,-1410.5201416016,28.292112350464,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Faça uma rápida ~r~parada~s~ e olhe para sua ~y~ESQUERDA~s~ antes de entrar no trânsito", 5000)
			PlaySound(-1, "RACE_PLACED", "HUD_AWARDS", 0, 0, 1)
			FreezeEntityPosition(GetVehiclePedIsUsing(GetPlayerPed(-1)), true) -- Freeze Entity
			Citizen.Wait(6000)
			FreezeEntityPosition(GetVehiclePedIsUsing(GetPlayerPed(-1)), false) -- Freeze Entity
			DrawMissionText2("~g~Ótimo!~s~ agora pegue a ~y~DIREITA~s~ e escolha sua pista", 5000)
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),178.55052185059,-1401.7551269531,27.725154876709, true) > 4.0001 then
		   DrawMarker(1,178.55052185059,-1401.7551269531,27.725154876709,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Veja os ~y~semáforos~s~ !", 5000)
			onTestEvent = 6
		end
	end	
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),-73.542953491211,-1364.3355712891,27.789325714111, true) > 4.0001 then
		   DrawMarker(1,-73.542953491211,-1364.3355712891,27.789325714111,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Certifique-se de parar, para passagem de veículos !", 5000)
			onTestEvent = 8
		end
	end			
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),-453.79092407227,-1444.7265625,27.665870666504, true) > 4.0001 then
		   DrawMarker(1,-453.79092407227,-1444.7265625,27.665870666504,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			...
			N_0x80ead8e2e1d5d52e(onTestBlipp)
			SetBlipRoute(onTestBlipp, 1)
		    DrawMissionText2("Na hora de entrar na estrada, veja sua velocidade e evite bater !", 5000)
			PlaySound(-1, "RACE_PLACED", "HUD_AWARDS", 0, 0, 1)
			drawNotification("Area: ~y~Freeway\n~s~Limite de velocidade: ~y~120 km/h\n~s~Pontos acumulados: ~y~".. Error.."/4")
			onTestEvent = 11
			SpeedControl = 3
			Citizen.Wait(4000)
		end
...
RegisterNUICallback('close', function(data, cb)
  ...
  DrawMissionText2("~b~Passou no teste, agora você pode passar ao teste de condução", 2000)	
  TestDone = 0
  onTtest = 3
end)
...
Citizen.CreateThread(function()
    while true do
		...
		DrawMissionText2("~r~Você está dirigindo sem licença", 2000)			
		end
	end
end)
            ...
            if IsControlJustPressed(1, 73) then
                if (GetPedInVehicleSeat(vehicle, -1) == ped) then
                    if CruiseControl == 0 then
						...
						drawNotification("~y~Controle de cruzeiro: ~g~ligado\n~s~Velocidade máxima ".. math.floor(speedLimit*3.6).."kmh")
						Citizen.Wait(1000)
				        DisplayHelpText("Ajuste sua velocidade máxima com setas ~INPUT_CELLPHONE_UP~ ~INPUT_CELLPHONE_DOWN~")
						PlaySound(-1, "COLLECTED", "HUD_AWARDS", 0, 0, 1)
                        CruiseControl = 1
                    else
                        SetEntityMaxSpeed(vehicle, Max)
						drawNotification("~y~Controle de cruzeiro: ~r~desligado")						
                        CruiseControl = 0
                    end
                else
	...
	while true do
		...
		for k,v in ipairs(dmvpedpos) do
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 1.0)then
				DisplayHelpText("Pressione ~INPUT_CONTEXT~ para interagir com ~y~NPC")
...
function VehLicenseMenu()
    ClearMenu()
    options.menu_title = "Carteira de Motorista"
	Menu.addButton("Exame teorico    200$","startttest",nil)
	Menu.addButton("Exame pratico    500$","startptest",nil)
    Menu.addButton("Voltar","DMVMenu",nil) 
end
